...
def test_suprimum_weights_normal_case():
    ...
    result = owa_suprimum_weights_linear(n)
...
def test_infimum_weights_normal_case():
    ...
    result = owa_infimum_weights_linear(n)
...
def test_single_element_case():
    ...
    assert np.array_equal(owa_suprimum_weights_linear(1), np.array([1.0]))
    assert np.array_equal(owa_infimum_weights_linear(1), np.array([1.0]))

@pytest.mark.parametrize("func", [
    owa_suprimum_weights_linear,
    owa_infimum_weights_linear
])
...
def test_sum_to_one_property_small_n(n):
    ...
    sup = owa_suprimum_weights_linear(n)
    inf = owa_infimum_weights_linear(n)
...
test_cases = [
    (owa_infimum_weights_linear, 5, "owa_infimum_weights_linear_len_5"),
    (owa_infimum_weights_linear, 10, "owa_infimum_weights_linear_len_10"),
    (owa_suprimum_weights_linear, 8, "owa_suprimum_weights_linear_len_8"),
    (owa_suprimum_weights_linear, 13, "owa_suprimum_weights_linear_len_13"),
]
