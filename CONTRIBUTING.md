...
Version-controlled scientific test oracles live in `tests/reference_data/` and
are covered by integrity checks and CODEOWNERS review rules. Read
[`tests/reference_data/README.md`](tests/reference_data/README.md) before editing
a reference-data JSON file, the manifest, the shared loader, or its compatibility
accessors.

Do not update expected values merely because an implementation test fails. A
reference-data change must be independently derived, scientifically justified,
documented in the file's provenance, accompanied by the updated SHA-256 entry in
`manifest.json`, and reviewed separately from production-code changes whenever
possible. Expected values must not be generated by the production implementation
that they are meant to validate, and numerical tolerances must not be relaxed
without an explicit justification.

Run the reference-data contract tests after any such change:

```bash
python -m pytest tests/reference_data_tests -ra
```

Also run every operator or model test module that consumes the affected dataset.
Approval from the designated code owner is required before merging changes to
this protected test-oracle surface.
