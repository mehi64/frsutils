"""
@file benchmark_oversamplers.py
@brief Benchmark FRSMOTE vs. selected SMOTE baselines from (1) imbalanced-learn and (2) smote-variants.
...
def build_samplers(
    ...
    samplers["ADASYN"] = TimedSamplerAdapter(ADASYN(random_state=random_state, n_neighbors=5, sampling_strategy="minority"))
...
def evaluate_fold(
    ...
    steps = []
    if use_scaler:
        sb = (scaler_before_sampler or "minmax").lower().strip()
        sa = (scaler_after_sampler or "standard").lower().strip()
...
def evaluate_keel_predefined_folds(
    ...
    """
    cfg = cfg.ensure_metrics()
    protocol = f"KEEL_PredefinedFolds(dir={keel_cv_dir})"
...
def run_task(task: DatasetTask, samplers: Dict[str, Optional[Any]], classifiers: Dict[str, Any], cfg: BenchmarkConfig) -> List[FoldResult]:
    """
...
def run_benchmark(tasks: List[DatasetTask], out_dir: Union[str, Path], cfg: BenchmarkConfig, frsmote_kwargs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    @brief Run benchmark for multiple dataset tasks and write clean outputs.
    ...
    @return dict with keys:
      - df_folds, df_dataset, summary_df, stats
      - output_paths
    """
    cfg = cfg.ensure_metrics()
    ...
    for i, t in enumerate(tasks, start=1):
        ...
        rows = run_task(t, samplers, classifiers, cfg)
        all_rows.extend(rows)

    df_folds = fold_results_to_dataframe(all_rows)

    metric_names = [m.name for m in (cfg.metrics or [])]
    df_dataset = aggregate_dataset_level(df_folds, metric_names)

    summary_df, stats = build_overall_summary(df_dataset, cfg=cfg, ref_sampler="FRSMOTE", block_cols=("dataset", "classifier"))
...
def main():
    ...
    run_benchmark(tasks, out_dir=args.out_dir, cfg=cfg, frsmote_kwargs=frsmote_kwargs)
