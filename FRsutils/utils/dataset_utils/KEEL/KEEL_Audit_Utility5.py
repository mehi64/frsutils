...
unclean, but also as suitable, fragile, or risky for neighbor-based oversampling.

##############################################
# ✅ Quick Summary of Features
# - audit_keel_file(...)         -> Full single-file audit with data quality + oversampling diagnostics
# - audit_keel_folder(...)       -> Folder/fold audit with leakage, schema drift, and partition stability
# - audit_keel_root(...)         -> Root-level executive summary + offender ranking + redundancy view
#
# General quality checks:
# - Header/schema parsing, encoding checks, duplicate rows, missingness, label conflicts
# - Constant / near-constant features, high-cardinality nominal features, numeric sanity checks
# - Train/test leakage, class-presence mismatches, schema drift across predefined folds
#
...
# - Overlap, separability, instance-hardness, and local noise suspicion proxies
# - Synthetic interpolation risk analysis and oversampling-specific recommendation layer
# - Fold-stability analysis for IR, minority count, topology, overlap, hardness, and risk
#
# Level-3 additions:
# - Per-dataset severity score (0..100) with dominant drivers
# - Per-dataset run readiness + oversampling recommendation
# - Global issue counters, top offenders, and dataset-profile similarity hints
#
##############################################
...
# Design Pattern          Strategy                Rule-based readiness and recommendation layer
# Architecture            Layered Auditing        File -> folder -> root composition
# Architecture            Data-Centric Diagnostics Quality, topology, overlap, noise, stability, recommendation
# Clean Code              SRP, DRY, Fail-Safe     Parsing/checking/summarizing separated; robust fallbacks
# Clean Code              Explainability          Severity drivers + explicit risky/blocked reasons
#
##############################################
# ✅ How to Use - Examples
##############################################
...
# print(folder_res["executive_dataset_summaries"][0]["partition_stability_summary"])
#
...
# ============================================================================
# Keep the original procedural implementations as stable internal engines.
_procedural_audit_keel_file = audit_keel_file
_procedural_audit_keel_folder = audit_keel_folder
_procedural_audit_keel_root = audit_keel_root
...
class AuditCategoryDescriptor:
    ...
    This descriptor is intentionally simple so that future dataset families can
    reuse the same category model while exposing different owned output sections.
    """
    key: str
    title: str
    description: str
...
class BaseAuditCategory:
    ...
    Each subclass documents a coherent group of analyses. The main goal is to
    make the library structure understandable and extensible even when some of
    the low-level numerical logic is still delegated to stable helper functions.
    """
    ...
    def build_metadata(self) -> Dict[str, Any]:
        """
        @brief Return serializable metadata describing this category.
        """
        d = self.descriptor
        return {
            "key": d.key,
...
class BaseKEELAuditFamily:
    ...
    A family owns:
    - the list of active audit categories
    - the file/folder/root audit engines
    - metadata attached to outputs for discoverability
    """
...
class _PlaceholderFutureFamily(BaseKEELAuditFamily):
    ...
    It is registered so that the library structure is ready for expansion, but
    it intentionally raises a clear error until that family is implemented.
    """
...
class ImbalancedOversamplingAuditFamily(BaseKEELAuditFamily):
    """
    @brief Active family for the current project focus: imbalanced classification
    datasets used in oversampling research.

    The numerical computations remain delegated to the stable procedural engine,
    while the public architecture is now category-based and family-aware.
    """
...
class KEELAuditFamilyRegistry:
    ...
    This is the single expansion point for future KEEL dataset families.
    """
...
class KEELAuditFacade:
    ...
    This keeps the old user-facing functions alive and makes the implementation
    extensible for future KEEL dataset categories.
    """
...
def audit_keel_file(
    ...
    *,
    ...
    """
    @brief Public stable API for file-level audit.

    The signature stays compatible with the previous API. A new optional
    dataset_family selector has been added with a default that preserves the
    current behavior.
    """
...
def audit_keel_folder(
    ...
    *,
    ...
    """
    @brief Public stable API for folder-level audit.
    """
...
def audit_keel_root(
    ...
    *,
    ...
    """
    @brief Public stable API for root-level audit.
    """
