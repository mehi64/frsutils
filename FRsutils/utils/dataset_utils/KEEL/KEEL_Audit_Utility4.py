...
# =============================================================================
# Enhanced audit extensions (article-oriented additions)
# =============================================================================

_AUDIT_PROFILE_DEFAULTS: Dict[str, Dict[str, Any]] = {
    "strict": {
        "near_constant_threshold": 0.995,
        "high_missingness_threshold": 0.20,
        "high_cardinality_threshold": 40,
        "robust_z_threshold": 3.0,
        "oversampling_analysis_max_rows": 10000,
        "oversampling_feasibility_ks": [3, 5, 7],
        "oversampling_topology_k": 5,
        "near_duplicate_numeric_round_decimals": 4,
        "near_duplicate_rate_threshold": 0.005,
        "feature_shift_numeric_smd_threshold": 0.35,
        "feature_shift_nominal_tvd_threshold": 0.20,
        "class_conditional_missingness_gap_threshold": 0.15,
        "feature_redundancy_corr_threshold": 0.98,
        "feature_redundancy_duplicate_ratio_threshold": 0.995,
        "minority_fragmentation_component_ratio_threshold": 0.35,
        "minority_fragmentation_singleton_ratio_threshold": 0.30,
        "unseen_category_rate_threshold": 0.01,
        "target_leakage_suspicion_threshold": 0.95,
        "majority_noise_rate_threshold": 0.20,
        "metric_min_minority_count_pr": 10,
        "metric_min_minority_count_f1": 5,
    },
    "balanced": {
        ...
        "oversampling_feasibility_ks": [3, 5, 7],
        "oversampling_topology_k": 5,
        "near_duplicate_numeric_round_decimals": 3,
        "near_duplicate_rate_threshold": 0.01,
        "feature_shift_numeric_smd_threshold": 0.50,
        "feature_shift_nominal_tvd_threshold": 0.25,
        "class_conditional_missingness_gap_threshold": 0.20,
        "feature_redundancy_corr_threshold": 0.995,
        "feature_redundancy_duplicate_ratio_threshold": 0.999,
        "minority_fragmentation_component_ratio_threshold": 0.45,
        "minority_fragmentation_singleton_ratio_threshold": 0.40,
        "unseen_category_rate_threshold": 0.02,
        "target_leakage_suspicion_threshold": 0.97,
        "majority_noise_rate_threshold": 0.25,
        "metric_min_minority_count_pr": 8,
        "metric_min_minority_count_f1": 4,
    },
    "lenient": {
        "near_constant_threshold": 0.9995,
        ...
        "oversampling_feasibility_ks": [3, 5, 7],
        "oversampling_topology_k": 5,
        "near_duplicate_numeric_round_decimals": 2,
        "near_duplicate_rate_threshold": 0.02,
        "feature_shift_numeric_smd_threshold": 0.65,
        "feature_shift_nominal_tvd_threshold": 0.35,
        "class_conditional_missingness_gap_threshold": 0.25,
        "feature_redundancy_corr_threshold": 0.999,
        "feature_redundancy_duplicate_ratio_threshold": 0.9995,
        "minority_fragmentation_component_ratio_threshold": 0.55,
        "minority_fragmentation_singleton_ratio_threshold": 0.50,
        "unseen_category_rate_threshold": 0.05,
        "target_leakage_suspicion_threshold": 0.985,
        "majority_noise_rate_threshold": 0.35,
        "metric_min_minority_count_pr": 5,
        "metric_min_minority_count_f1": 3,
    },
}

_AUDIT_CONTEXT_STACK: List[Dict[str, Any]] = []
...
def _resolve_audit_settings(
    *,
    ...
    robust_z_threshold: Optional[float] = None,
    oversampling_analysis_max_rows: Optional[int] = None,
    oversampling_feasibility_ks: Optional[List[int]] = None,
    oversampling_topology_k: Optional[int] = None,
) -> Dict[str, Any]:
    """
    @brief Resolve threshold settings from profile + overrides + explicit args.
    """
    profile = (audit_profile or "balanced").strip().lower()
    if profile not in _AUDIT_PROFILE_DEFAULTS:
        profile = "balanced"
    settings = dict(_AUDIT_PROFILE_DEFAULTS[profile])
    ...
    settings["audit_profile"] = profile
    settings["oversampling_feasibility_ks"] = sorted(list(dict.fromkeys(settings.get("oversampling_feasibility_ks", [3, 5, 7]))))
    return settings


def _push_audit_context(settings: Dict[str, Any]) -> None:
    _AUDIT_CONTEXT_STACK.append(dict(settings))


def _pop_audit_context() -> None:
    if _AUDIT_CONTEXT_STACK:
        _AUDIT_CONTEXT_STACK.pop()


def _get_current_audit_context() -> Optional[Dict[str, Any]]:
    return dict(_AUDIT_CONTEXT_STACK[-1]) if _AUDIT_CONTEXT_STACK else None


def _load_keel_dataframe_for_enhanced_checks(file_path: str) -> Tuple[str, Dict[str, Any], List[str], List[str], pd.DataFrame]:
    """
    @brief Load KEEL file into (relation, attributes, inputs, outputs, raw DataFrame).
    """
    ...
    if not inputs:
        inputs = [c for c in attr_names if c not in outputs]
    return relation, attributes, inputs, outputs, df


def _normalize_token_for_compare(value: Any) -> str:
    if value is None:
        return "__NONE__"
    s = str(value).strip()
    return "__MISSING__" if _is_missing_token(s) else s.lower()


def _near_duplicate_signature(values: List[Any], attr_names: List[str], attributes: Dict[str, Any], decimals: int) -> str:
    """
    @brief Build a coarse signature for near-duplicate detection.
    """
    parts = []
    for col, val in zip(attr_names, values):
        sval = str(val).strip()
        if _is_missing_token(sval):
            parts.append("__MISSING__")
            continue
        atype = attributes.get(col, {}).get("type")
        if atype in {"real", "integer"}:
            try:
                parts.append(f"{round(float(sval), int(decimals)):.{int(decimals)}f}")
            except Exception:
                parts.append(sval.lower())
        else:
            parts.append(sval.lower())
    return _row_hash(parts)


def _compute_class_conditional_missingness(
    df: pd.DataFrame,
    *,
    inputs: List[str],
    outputs: List[str],
    ...
    """
    @brief Compare per-class missingness rates for each input feature.
    """
    if len(outputs) != 1 or outputs[0] not in df.columns:
        return {"enabled": False, "reason": "multioutput_or_unknown", "flagged_features": []}

    target = outputs[0]
    y = df[target].astype(str).map(lambda x: str(x).strip())
    valid_mask = ~y.map(_is_missing_token)
    if not valid_mask.any():
        return {"enabled": False, "reason": "no_valid_target_rows", "flagged_features": []}

    work = df.loc[valid_mask, inputs].copy()
    ...
    for col in inputs:
        ...
        for cls in classes:
            ...
            if denom == 0:
                continue
            vals = work.loc[mask, col].astype(str).tolist()
            rates[str(cls)] = float(sum(_is_missing_token(v) for v in vals) / denom)
        vals = list(rates.values())
        gap = float(max(vals) - min(vals)) if vals else float("nan")
        per_feature[col] = {"per_class_missingness": rates, "max_gap": gap}
        if np.isfinite(gap) and gap >= float(gap_threshold):
            flagged.append({"feature": col, "max_gap": gap, "per_class_missingness": rates})

    flagged = sorted(flagged, key=lambda x: x.get("max_gap", 0.0), reverse=True)
    return {
        "enabled": True,
        "gap_threshold": float(gap_threshold),
        "n_flagged_features": int(len(flagged)),
        "flagged_features": flagged,
        "per_feature": per_feature,
    }


def _compute_feature_redundancy_checks(
    df: pd.DataFrame,
    *,
    attributes: Dict[str, Any],
    inputs: List[str],
    ...
    """
    @brief Detect redundant features via high correlation and duplicated columns.
    """
    numeric_inputs = [c for c in inputs if attributes.get(c, {}).get("type") in {"real", "integer"}]
    nominal_inputs = [c for c in inputs if attributes.get(c, {}).get("type") == "nominal"]

    high_corr_pairs = []
    if len(numeric_inputs) >= 2:
        num_df = df[numeric_inputs].replace(list(_MISSING_TOKENS), np.nan).apply(pd.to_numeric, errors="coerce")
        if num_df.shape[1] >= 2:
            corr = num_df.corr().abs()
            for i, c1 in enumerate(corr.columns):
                for c2 in corr.columns[i + 1:]:
                    val = corr.loc[c1, c2]
                    if isinstance(val, (int, float)) and np.isfinite(val) and val >= float(corr_threshold):
                        high_corr_pairs.append({"feature_a": c1, "feature_b": c2, "abs_corr": float(val)})

    exact_duplicate_columns = []
    near_duplicate_columns = []
    normalized_cache = {}
    for col in inputs:
        normalized_cache[col] = df[col].astype(str).map(_normalize_token_for_compare)

    for i, c1 in enumerate(inputs):
        s1 = normalized_cache[c1]
        for c2 in inputs[i + 1:]:
            s2 = normalized_cache[c2]
            eq_ratio = float(np.mean((s1 == s2).to_numpy())) if len(s1) > 0 else float("nan")
            if np.isfinite(eq_ratio) and eq_ratio >= 1.0:
                exact_duplicate_columns.append({"feature_a": c1, "feature_b": c2, "equality_ratio": eq_ratio})
            elif np.isfinite(eq_ratio) and eq_ratio >= float(duplicate_ratio_threshold):
                near_duplicate_columns.append({"feature_a": c1, "feature_b": c2, "equality_ratio": eq_ratio})

    return {
        "enabled": True,
        "corr_threshold": float(corr_threshold),
        "duplicate_ratio_threshold": float(duplicate_ratio_threshold),
        "high_corr_pairs": high_corr_pairs,
        "exact_duplicate_columns": exact_duplicate_columns,
        "near_duplicate_columns": near_duplicate_columns,
        "has_redundancy_signal": bool(high_corr_pairs or exact_duplicate_columns or near_duplicate_columns),
    }


def _binary_best_threshold_accuracy(y_bin: np.ndarray, x: np.ndarray) -> float:
    """
    @brief Best simple threshold accuracy proxy for target-leakage suspicion.
    """
    if x.size == 0 or y_bin.size == 0 or x.size != y_bin.size:
        return float("nan")
    qs = np.unique(np.quantile(x, np.linspace(0.05, 0.95, 15)))
    if qs.size == 0:
        return float("nan")
    best = 0.0
    for thr in qs:
        pred1 = (x >= thr).astype(int)
        pred2 = (x < thr).astype(int)
        best = max(best, float(np.mean(pred1 == y_bin)), float(np.mean(pred2 == y_bin)))
    return best


def _compute_target_leakage_suspicion(
    df: pd.DataFrame,
    *,
    attributes: Dict[str, Any],
    inputs: List[str],
    ...
    """
    @brief Detect suspiciously target-revealing input features.
    """
    if len(outputs) != 1 or outputs[0] not in df.columns:
        return {"enabled": False, "reason": "multioutput_or_unknown", "flagged_features": []}
    ...
    for col in inputs:
        atype = attributes.get(col, {}).get("type")
        ser = work[col].astype(str).map(lambda x: str(x).strip())
        nonmissing_mask = ~ser.map(_is_missing_token)
        ser_nm = ser.loc[nonmissing_mask]
        y_nm = y.loc[nonmissing_mask]
        if ser_nm.empty:
            continue
        ...
        if ser_nm.equals(y_nm.astype(str)):
            suspicion_score = 1.0
            reason = "feature_matches_target_exactly"
        elif atype == "nominal":
            table = pd.crosstab(ser_nm, y_nm)
            purity = float(table.max(axis=1).sum() / table.to_numpy().sum()) if table.to_numpy().sum() > 0 else float("nan")
            suspicion_score = purity if np.isfinite(purity) else 0.0
            reason = "high_single_feature_class_purity"
            details = {"purity": purity, "n_unique_feature_values": int(ser_nm.nunique())}
        elif atype in {"real", "integer"} and len(classes) == 2:
            x = pd.to_numeric(ser_nm, errors="coerce")
            mask = x.notna()
            x = x.loc[mask].to_numpy(dtype=float)
            y_bin = (y_nm.loc[mask].astype(str) == classes[-1]).astype(int).to_numpy(dtype=int)
            score = _binary_best_threshold_accuracy(y_bin, x)
            suspicion_score = score if np.isfinite(score) else 0.0
            reason = "single_numeric_threshold_predicts_target"
            details = {"best_threshold_accuracy": score}
        else:
            table = pd.crosstab(pd.qcut(pd.to_numeric(ser_nm, errors="coerce"), q=min(10, max(2, ser_nm.nunique())), duplicates="drop"), y_nm)
            if not table.empty and table.to_numpy().sum() > 0:
                purity = float(table.max(axis=1).sum() / table.to_numpy().sum())
                suspicion_score = purity
                reason = "binned_numeric_class_purity"
                details = {"purity": purity}

        per_feature[col] = {"suspicion_score": float(suspicion_score), "reason": reason, **details}
        if suspicion_score >= float(suspicion_threshold):
            flagged.append({"feature": col, "suspicion_score": float(suspicion_score), "reason": reason, **details})

    flagged = sorted(flagged, key=lambda x: x.get("suspicion_score", 0.0), reverse=True)
    return {
        "enabled": True,
...
def _compute_metric_readiness(
    *,
    class_info: Dict[str, Any],
    oversampling_audit: Dict[str, Any],
    partition_stability: Optional[Dict[str, Any]] = None,
    min_minority_count_pr: int = 8,
    min_minority_count_f1: int = 4,
) -> Dict[str, Any]:
    """
    @brief Heuristic readiness of common imbalance metrics.
    """
    class_type = class_info.get("class_type")
    min_class_count = class_info.get("min_class_count")
    min_class_count = int(min_class_count) if isinstance(min_class_count, (int, float)) and np.isfinite(min_class_count) else None
    unstable = bool(isinstance(partition_stability, dict) and not partition_stability.get("is_stable", True))
    risk = oversampling_audit.get("synthetic_interpolation_risk", {}).get("score_0_100") if isinstance(oversampling_audit, dict) else None
    risk = float(risk) if isinstance(risk, (int, float)) and np.isfinite(risk) else 0.0

    def make(status: str, reasons: List[str]) -> Dict[str, Any]:
        return {"status": status, "reasons": reasons}

    ready = {
        "balanced_accuracy": make("READY", []),
        "mcc": make("READY", []),
        "f1_macro": make("READY", []),
        "minority_f1": make("READY", []),
        "pr_auc": make("READY", []),
        "roc_auc": make("READY", []),
    }

    if class_type == "multioutput":
        for k in ready:
            ready[k] = make("NOT_RECOMMENDED", ["multioutput_target"])
        return ready

    if class_type != "binary":
        ready["pr_auc"] = make("USE_WITH_CAUTION", ["nonbinary_target_requires_explicit_multiclass_definition"])
        ready["roc_auc"] = make("USE_WITH_CAUTION", ["nonbinary_target_requires_explicit_multiclass_definition"])

    if min_class_count is not None and min_class_count < int(min_minority_count_pr):
        ready["pr_auc"] = make("USE_WITH_CAUTION", [f"minority_count_lt_{int(min_minority_count_pr)}"])
    if min_class_count is not None and min_class_count < int(min_minority_count_f1):
        ready["minority_f1"] = make("USE_WITH_CAUTION", [f"minority_count_lt_{int(min_minority_count_f1)}"])
        ready["f1_macro"] = make("USE_WITH_CAUTION", [f"minority_count_lt_{int(min_minority_count_f1)}"])

    if unstable:
        for k in ready:
            if ready[k]["status"] == "READY":
                ready[k] = make("USE_WITH_CAUTION", ["partition_instability"])
            elif "partition_instability" not in ready[k]["reasons"]:
                ready[k]["reasons"].append("partition_instability")

    if risk >= 75.0:
        for k in ["minority_f1", "pr_auc", "roc_auc"]:
            if ready[k]["status"] == "READY":
                ready[k] = make("USE_WITH_CAUTION", ["high_oversampling_risk_profile"])
            elif "high_oversampling_risk_profile" not in ready[k]["reasons"]:
                ready[k]["reasons"].append("high_oversampling_risk_profile")

    return ready


def _compute_pairwise_shift_and_category_risk(
    train_file: str,
    test_file: str,
    *,
    numeric_smd_threshold: float,
    nominal_tvd_threshold: float,
    unseen_category_rate_threshold: float,
) -> Dict[str, Any]:
    """
    @brief Compute feature-level train/test shift and unseen-category risk.
    """
    _, attrs_t, inputs_t, outputs_t, df_train = _load_keel_dataframe_for_enhanced_checks(train_file)
    _, attrs_s, inputs_s, outputs_s, df_test = _load_keel_dataframe_for_enhanced_checks(test_file)
    inputs = [c for c in inputs_t if c in inputs_s]
    attrs = attrs_t if attrs_t else attrs_s
    ...
    for col in inputs:
        atype = attrs.get(col, {}).get("type")
        if atype in {"real", "integer"}:
            x1 = pd.to_numeric(df_train[col].replace(list(_MISSING_TOKENS), np.nan), errors="coerce").dropna().to_numpy(dtype=float)
            x2 = pd.to_numeric(df_test[col].replace(list(_MISSING_TOKENS), np.nan), errors="coerce").dropna().to_numpy(dtype=float)
            if x1.size == 0 or x2.size == 0:
                continue
            s1 = float(np.std(x1, ddof=1)) if x1.size > 1 else 0.0
            s2 = float(np.std(x2, ddof=1)) if x2.size > 1 else 0.0
            pooled = float(np.sqrt((s1 ** 2 + s2 ** 2) / 2.0)) if (s1 > 0 or s2 > 0) else 0.0
            smd = float(abs(np.mean(x1) - np.mean(x2)) / pooled) if pooled > 0 else 0.0
            per_feature[col] = {"type": "numeric", "abs_standardized_mean_difference": smd}
            if smd >= float(numeric_smd_threshold):
                numeric_flagged.append({"feature": col, "abs_standardized_mean_difference": smd})
        elif atype == "nominal":
            s1 = df_train[col].astype(str).map(_normalize_token_for_compare)
            s2 = df_test[col].astype(str).map(_normalize_token_for_compare)
            cats = sorted(list(set(s1.tolist()) | set(s2.tolist())))
            if not cats:
                continue
            p1 = s1.value_counts(normalize=True).to_dict()
            p2 = s2.value_counts(normalize=True).to_dict()
            tvd = 0.5 * sum(abs(float(p1.get(c, 0.0)) - float(p2.get(c, 0.0))) for c in cats)
            unseen_test = sorted([c for c in set(s2.tolist()) if c not in set(s1.tolist()) and c != "__MISSING__"])
            unseen_rate = float(sum(s2.isin(unseen_test)) / len(s2)) if len(s2) > 0 and unseen_test else 0.0
            per_feature[col] = {
                "type": "nominal",
                "total_variation_distance": float(tvd),
                "unseen_categories_in_test": unseen_test,
                "unseen_category_rate_in_test": unseen_rate,
            }
            if tvd >= float(nominal_tvd_threshold):
                nominal_flagged.append({"feature": col, "total_variation_distance": float(tvd)})
            if unseen_test and unseen_rate >= float(unseen_category_rate_threshold):
                unseen_risks.append({"feature": col, "unseen_categories_in_test": unseen_test, "unseen_category_rate_in_test": unseen_rate})

    return {
        "numeric_smd_threshold": float(numeric_smd_threshold),
        "nominal_tvd_threshold": float(nominal_tvd_threshold),
        "unseen_category_rate_threshold": float(unseen_category_rate_threshold),
        "flagged_numeric_shift_features": sorted(numeric_flagged, key=lambda x: x.get("abs_standardized_mean_difference", 0.0), reverse=True),
        "flagged_nominal_shift_features": sorted(nominal_flagged, key=lambda x: x.get("total_variation_distance", 0.0), reverse=True),
        "unseen_test_category_risks": unseen_risks,
        "per_feature": per_feature,
    }


_oversampling_knn_diagnostics_base = _oversampling_knn_diagnostics
_compute_leakage_metrics_from_hashes_base = _compute_leakage_metrics_from_hashes
_aggregate_dataset_key_summary_base = _aggregate_dataset_key_summary
_run_readiness_base = _run_readiness
_severity_score_from_flags_base = _severity_score_from_flags
_oversampling_dataset_recommendation_base = _oversampling_dataset_recommendation
_audit_keel_file_base = audit_keel_file
_audit_keel_folder_base = audit_keel_folder
_audit_keel_root_base = audit_keel_root
...
def _oversampling_knn_diagnostics(
    ...
    *,
    ...
    """
    @brief Enhanced oversampling diagnostics with majority-noise and fragmentation proxies.
    """
    base = _oversampling_knn_diagnostics_base(
        df,
        attributes=attributes,
        inputs=inputs,
        outputs=outputs,
        k_neighbors=k_neighbors,
        feasibility_ks=feasibility_ks,
        max_rows=max_rows,
    )
    ...
    y = prep["y"]
    minority_label = prep["minority_label"]
    majority_label = prep["majority_label"]
    minority_mask = (y == minority_label)
    ...
    n_neighbors = int(min(max(2, k_neighbors + 1), max(2, n_total - 1)))
    ...
    majority_noise_flags = []
    for i in np.where(majority_mask)[0]:
        n_labels = y[neigh_idx[i]]
        minority_ratio = float(np.mean(n_labels == minority_label))
        majority_noise_flags.append(bool(minority_ratio >= 0.80))

    base.setdefault("noise_suspicion", {})
    base["noise_suspicion"].update({
        "majority_noise_suspicion_count": int(sum(majority_noise_flags)),
        "majority_noise_suspicion_rate": float(np.mean(majority_noise_flags)) if majority_noise_flags else 0.0,
    })

    minority_idx = np.where(minority_mask)[0]
    comp_info = {"n_components": 0, "singleton_components": 0, "component_sizes": [], "component_ratio_over_minority": float("nan"), "singleton_ratio_over_minority": float("nan")}
    if minority_idx.size > 0:
        if minority_idx.size == 1:
            comp_info = {"n_components": 1, "singleton_components": 1, "component_sizes": [1], "component_ratio_over_minority": 1.0, "singleton_ratio_over_minority": 1.0}
        elif minority_idx.size == 2:
            comp_info = {"n_components": 1, "singleton_components": 0, "component_sizes": [2], "component_ratio_over_minority": 0.5, "singleton_ratio_over_minority": 0.0}
        else:
            m_k = int(min(3, max(1, minority_idx.size - 1)))
            xm = x[minority_idx]
            nn_m = NearestNeighbors(n_neighbors=min(m_k + 1, minority_idx.size - 1), metric="euclidean")
            nn_m.fit(xm)
            neigh_m = nn_m.kneighbors(return_distance=False)
            graph = {i: set() for i in range(minority_idx.size)}
            for i in range(minority_idx.size):
                for j in neigh_m[i]:
                    if int(j) == i:
                        continue
                    graph[i].add(int(j))
            ...
            comp_info = {
                ...
                "singleton_components": int(sum(1 for s in sizes if s == 1)),
                "component_sizes": sizes,
                "component_ratio_over_minority": float(len(sizes) / minority_idx.size),
                "singleton_ratio_over_minority": float(sum(1 for s in sizes if s == 1) / minority_idx.size),
            }
    base["minority_fragmentation"] = comp_info

    rec = base.get("oversampling_recommendation", {}) if isinstance(base.get("oversampling_recommendation"), dict) else {"status": "GOOD_FOR_STANDARD_OVERSAMPLING", "reasons": []}
    reasons = list(rec.get("reasons", []))
    if comp_info.get("component_ratio_over_minority", 0.0) >= 0.45:
        reasons.append("minority_fragmentation_high")
    if base.get("noise_suspicion", {}).get("majority_noise_suspicion_rate", 0.0) >= 0.25:
        reasons.append("high_majority_noise_suspicion")
    rec["reasons"] = sorted(list(dict.fromkeys(reasons)))
    if rec.get("status") == "GOOD_FOR_STANDARD_OVERSAMPLING" and rec["reasons"]:
        rec["status"] = "USE_WITH_CAUTION"
    base["oversampling_recommendation"] = rec
...
def _compute_leakage_metrics_from_hashes(train_file: str, test_file: str, *, inputs: List[str], outputs: List[str]) -> Dict[str, Any]:
    """
    @brief Enhanced leakage metrics including near-duplicate leakage and feature shift.
    """
    base = _compute_leakage_metrics_from_hashes_base(train_file, test_file, inputs=inputs, outputs=outputs)
    ctx = _get_current_audit_context() or _resolve_audit_settings(audit_profile="balanced")
    decimals = int(ctx.get("near_duplicate_numeric_round_decimals", 3))

    _, attributes_t, inputs_t, _, df_train = _load_keel_dataframe_for_enhanced_checks(train_file)
    _, attributes_s, inputs_s, _, df_test = _load_keel_dataframe_for_enhanced_checks(test_file)
    shared_inputs = [c for c in inputs if (c in df_train.columns and c in df_test.columns)] or [c for c in inputs_t if c in inputs_s]
    attributes = attributes_t if attributes_t else attributes_s

    train_signatures = set()
    for _, row in df_train[shared_inputs].iterrows():
        train_signatures.add(_near_duplicate_signature(row.tolist(), shared_inputs, attributes, decimals))
    test_signatures = set()
    for _, row in df_test[shared_inputs].iterrows():
        test_signatures.add(_near_duplicate_signature(row.tolist(), shared_inputs, attributes, decimals))
    inter_near = int(len(train_signatures.intersection(test_signatures)))
    base["n_intersection_inputs_near_duplicate"] = inter_near
    base["has_near_duplicate_leakage"] = bool(inter_near > 0)

    shift = _compute_pairwise_shift_and_category_risk(
        train_file,
        test_file,
        numeric_smd_threshold=float(ctx.get("feature_shift_numeric_smd_threshold", 0.50)),
        nominal_tvd_threshold=float(ctx.get("feature_shift_nominal_tvd_threshold", 0.25)),
        unseen_category_rate_threshold=float(ctx.get("unseen_category_rate_threshold", 0.02)),
    )
    base["feature_shift"] = shift
    base["has_feature_distribution_shift"] = bool(shift.get("flagged_numeric_shift_features") or shift.get("flagged_nominal_shift_features"))
    base["has_unseen_test_categories"] = bool(shift.get("unseen_test_category_risks"))
    return base


def _severity_score_from_flags(flags: Dict[str, Any]) -> Dict[str, Any]:
    """
    @brief Compute a 0..100 severity score with expanded factors.
    """
    base = _severity_score_from_flags_base(flags)
    weights = {
        "near_duplicate_leakage": 25,
        "feature_distribution_shift": 12,
        "class_conditional_missingness_gap": 8,
        "feature_redundancy": 5,
        "minority_fragmentation": 10,
        "unseen_test_categories": 10,
        "target_leakage_suspicion": 35,
        "high_majority_noise_suspicion": 8,
        "metric_readiness_risk": 6,
    }
    breakdown = dict(base.get("breakdown", {}))
    total = int(base.get("score_0_100", 0))
    ...
    add("near_duplicate_leakage", bool(flags.get("has_near_duplicate_leakage", False)))
    add("feature_distribution_shift", bool(flags.get("has_feature_distribution_shift", False)))
    add("class_conditional_missingness_gap", bool(flags.get("has_class_conditional_missingness_gap", False)))
    add("feature_redundancy", bool(flags.get("has_feature_redundancy", False)))
    add("minority_fragmentation", bool(flags.get("has_minority_fragmentation", False)))
    add("unseen_test_categories", bool(flags.get("has_unseen_test_categories", False)))
    add("target_leakage_suspicion", bool(flags.get("has_target_leakage_suspicion", False)))
    add("high_majority_noise_suspicion", bool(flags.get("has_high_majority_noise_suspicion", False)))
    add("metric_readiness_risk", bool(flags.get("has_metric_readiness_risk", False)))

    base["breakdown"] = breakdown
    base["score_0_100"] = int(min(100, total))
    return base


def _run_readiness(flags: Dict[str, Any]) -> Dict[str, Any]:
    """
    @brief Expanded SAFE/RISKY/BLOCKED readiness computation.
    """
    base = _run_readiness_base(flags)
    blocked = list(base.get("blocked_reasons", []))
    risky = list(base.get("risky_reasons", []))

    if flags.get("has_target_leakage_suspicion", False):
        blocked.append("target_leakage_suspicion")
    if flags.get("has_near_duplicate_leakage", False) and "train_test_leakage" not in blocked:
        blocked.append("near_duplicate_leakage")

    if flags.get("has_feature_distribution_shift", False):
        risky.append("feature_distribution_shift")
    if flags.get("has_class_conditional_missingness_gap", False):
        risky.append("class_conditional_missingness_gap")
    if flags.get("has_feature_redundancy", False):
        risky.append("feature_redundancy")
    if flags.get("has_minority_fragmentation", False):
        risky.append("minority_fragmentation")
    if flags.get("has_unseen_test_categories", False):
        risky.append("unseen_test_categories")
    if flags.get("has_high_majority_noise_suspicion", False):
        risky.append("high_majority_noise_suspicion")
    if flags.get("has_metric_readiness_risk", False):
        risky.append("metric_readiness_risk")

    blocked = sorted(list(dict.fromkeys(blocked)))
    risky = sorted(list(dict.fromkeys(risky)))
    status = "BLOCKED" if blocked else ("RISKY" if risky else "SAFE")
    return {"status": status, "blocked_reasons": blocked, "risky_reasons": risky}


def _oversampling_dataset_recommendation(flags: Dict[str, Any], *, partition_stability: Optional[Dict[str, Any]] = None, representative_audit: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    @brief Expanded dataset recommendation for oversampling studies.
    """
    base = _oversampling_dataset_recommendation_base(flags, partition_stability=partition_stability, representative_audit=representative_audit)
    blocked = list(base.get("blocked_reasons", []))
    cautious = list(base.get("cautious_reasons", []))
    actions = list(base.get("preferred_actions", []))

    if flags.get("has_target_leakage_suspicion", False):
        blocked.append("target_leakage_suspicion")
    if flags.get("has_near_duplicate_leakage", False):
        blocked.append("near_duplicate_leakage")
    if flags.get("has_feature_distribution_shift", False):
        cautious.append("feature_distribution_shift")
    if flags.get("has_class_conditional_missingness_gap", False):
        cautious.append("class_conditional_missingness_gap")
    if flags.get("has_feature_redundancy", False):
        cautious.append("feature_redundancy")
    if flags.get("has_minority_fragmentation", False):
        cautious.append("minority_fragmentation")
    if flags.get("has_unseen_test_categories", False):
        cautious.append("unseen_test_categories")
    if flags.get("has_high_majority_noise_suspicion", False):
        cautious.append("high_majority_noise_suspicion")

    blocked = sorted(list(dict.fromkeys(blocked)))
    cautious = sorted(list(dict.fromkeys(cautious)))
    status = "NOT_SUITABLE_FOR_STANDARD_OVERSAMPLING" if blocked else ("USE_WITH_CAUTION" if cautious else "GOOD_FOR_STANDARD_OVERSAMPLING")

    if flags.get("has_target_leakage_suspicion", False):
        actions.extend(["exclude_from_suite_or_fix_leakage", "manual_feature_review"])
    if flags.get("has_feature_distribution_shift", False) or flags.get("has_unseen_test_categories", False):
        actions.extend(["report_train_test_shift_explicitly", "prefer_shift-aware_split_review"])
    if flags.get("has_minority_fragmentation", False):
        actions.extend(["prefer_borderline_or_filtered_oversamplers", "inspect_minority_subclusters"])

    base["status"] = status
    base["blocked_reasons"] = blocked
    base["cautious_reasons"] = cautious
    base["preferred_actions"] = sorted(list(dict.fromkeys(actions)))
    return base
...
def _aggregate_dataset_key_summary(
    *,
    ...
    """
    @brief Enhanced dataset summary with new oversampling-oriented checks.
    """
    row = _aggregate_dataset_key_summary_base(
        folder_name=folder_name,
        dataset_key=dataset_key,
        group_basenames=group_basenames,
        per_file_by_name=per_file_by_name,
        leakage_reports=leakage_reports,
        schema_drift_entry=schema_drift_entry,
        class_presence_issues=class_presence_issues,
    )

    audits = [per_file_by_name.get(bn) for bn in group_basenames]
    audits = [a for a in audits if isinstance(a, dict) and a.get("status") != "error"]
    agg_audits = [a for a in audits if a.get("file", {}).get("is_train")] or audits
    flags = dict(row.get("flags", {}))

    flags["has_class_conditional_missingness_gap"] = any(
        int(a.get("data_quality", {}).get("class_conditional_missingness", {}).get("n_flagged_features", 0)) > 0
        for a in agg_audits
    )
    flags["has_feature_redundancy"] = any(
        bool(a.get("feature_quality", {}).get("redundancy", {}).get("has_redundancy_signal", False))
        for a in agg_audits
    )
    flags["has_target_leakage_suspicion"] = any(
        int(a.get("target_leakage_suspicion", {}).get("n_flagged_features", 0)) > 0
        for a in agg_audits
    )
    flags["has_high_majority_noise_suspicion"] = any(
        float(a.get("oversampling_audit", {}).get("noise_suspicion", {}).get("majority_noise_suspicion_rate", 0.0)) >= 0.25
        for a in agg_audits
    )
    flags["has_minority_fragmentation"] = any(
        float(a.get("oversampling_audit", {}).get("minority_fragmentation", {}).get("component_ratio_over_minority", 0.0)) >= 0.45
        or float(a.get("oversampling_audit", {}).get("minority_fragmentation", {}).get("singleton_ratio_over_minority", 0.0)) >= 0.40
        for a in agg_audits
    )
    flags["has_metric_readiness_risk"] = any(
        any(v.get("status") != "READY" for v in (a.get("metric_readiness", {}) or {}).values() if isinstance(v, dict))
        for a in agg_audits
    )

    leaks = [x for x in leakage_reports if x.get("dataset_key") == dataset_key]
    flags["has_near_duplicate_leakage"] = any(bool(x.get("has_near_duplicate_leakage", False)) for x in leaks)
    flags["has_feature_distribution_shift"] = any(bool(x.get("has_feature_distribution_shift", False)) for x in leaks)
    flags["has_unseen_test_categories"] = any(bool(x.get("has_unseen_test_categories", False)) for x in leaks)

    worst_near_dup = max((int(x.get("n_intersection_inputs_near_duplicate", 0)) for x in leaks), default=0)
    shift_feature_count = max((len(x.get("feature_shift", {}).get("flagged_numeric_shift_features", [])) + len(x.get("feature_shift", {}).get("flagged_nominal_shift_features", [])) for x in leaks), default=0)
    unseen_feature_count = max((len(x.get("feature_shift", {}).get("unseen_test_category_risks", [])) for x in leaks), default=0)

    row.setdefault("leakage_summary", {})
    row["leakage_summary"].update({
        "worst_n_intersection_inputs_near_duplicate": int(worst_near_dup),
    })
    row["train_test_shift_summary"] = {
        "has_feature_distribution_shift": bool(flags["has_feature_distribution_shift"]),
        "has_unseen_test_categories": bool(flags["has_unseen_test_categories"]),
        "max_flagged_shift_features_in_a_fold": int(shift_feature_count),
        "max_unseen_category_risk_features_in_a_fold": int(unseen_feature_count),
        "fold_reports": leaks,
    }
    ...
    fragmentation = []
    majority_noise_rates = []
    metric_readiness = []
    for a in agg_audits:
        ccm = a.get("data_quality", {}).get("class_conditional_missingness", {})
        for it in ccm.get("flagged_features", [])[:10]:
            ccm_flagged.append({"file": a.get("file", {}).get("basename"), **it})
        red = a.get("feature_quality", {}).get("redundancy", {})
        for pair in (red.get("high_corr_pairs", [])[:10] + red.get("exact_duplicate_columns", [])[:10] + red.get("near_duplicate_columns", [])[:10]):
            redundancy_pairs.append({"file": a.get("file", {}).get("basename"), **pair})
        tl = a.get("target_leakage_suspicion", {})
        for it in tl.get("flagged_features", [])[:10]:
            target_leakage_features.append({"file": a.get("file", {}).get("basename"), **it})
        frag = a.get("oversampling_audit", {}).get("minority_fragmentation", {})
        if frag:
            fragmentation.append({"file": a.get("file", {}).get("basename"), **frag})
        mnr = a.get("oversampling_audit", {}).get("noise_suspicion", {}).get("majority_noise_suspicion_rate")
        if isinstance(mnr, (int, float)) and np.isfinite(mnr):
            majority_noise_rates.append(float(mnr))
        metric_readiness.append({"file": a.get("file", {}).get("basename"), "metrics": a.get("metric_readiness", {})})

    row["feature_quality_summary"].update({
        "class_conditional_missingness_flagged": ccm_flagged[:20],
        "redundancy_signals": redundancy_pairs[:20],
    })
    row["oversampling_summary"].update({
        "majority_noise_suspicion_rate_mean": _safe_mean(majority_noise_rates),
        "minority_fragmentation_examples": fragmentation[:10],
    })
    row["target_leakage_summary"] = {
        "has_target_leakage_suspicion": bool(flags["has_target_leakage_suspicion"]),
        "flagged_features": target_leakage_features[:20],
    }
    row["metric_readiness_summary"] = {"per_file": metric_readiness}

    row["flags"] = flags
    severity = _severity_score_from_flags(flags)
    severity["dominant_drivers"] = _dominant_severity_drivers(severity.get("breakdown", {}))
    row["severity"] = severity
    row["run_readiness"] = _run_readiness(flags)
    row["oversampling_summary"]["dataset_recommendation"] = _oversampling_dataset_recommendation(
        flags,
        partition_stability=row.get("partition_stability_summary"),
        representative_audit=agg_audits[0] if agg_audits else None,
    )
    return row
...
def audit_keel_file(
    ...
    *,
    ...
    """
    @brief Enhanced single-file audit with additional data-quality and benchmark-risk checks.
    """
    ctx = _get_current_audit_context()
    settings = _resolve_audit_settings(
        audit_profile=audit_profile or (ctx or {}).get("audit_profile"),
        profile_overrides=profile_overrides or (ctx or {}).get("profile_overrides"),
        near_constant_threshold=near_constant_threshold if near_constant_threshold is not None else (ctx or {}).get("near_constant_threshold"),
        high_missingness_threshold=high_missingness_threshold if high_missingness_threshold is not None else (ctx or {}).get("high_missingness_threshold"),
        high_cardinality_threshold=high_cardinality_threshold if high_cardinality_threshold is not None else (ctx or {}).get("high_cardinality_threshold"),
        robust_z_threshold=robust_z_threshold if robust_z_threshold is not None else (ctx or {}).get("robust_z_threshold"),
        oversampling_analysis_max_rows=oversampling_analysis_max_rows if oversampling_analysis_max_rows is not None else (ctx or {}).get("oversampling_analysis_max_rows"),
        oversampling_feasibility_ks=oversampling_feasibility_ks if oversampling_feasibility_ks is not None else (ctx or {}).get("oversampling_feasibility_ks"),
        oversampling_topology_k=oversampling_topology_k if oversampling_topology_k is not None else (ctx or {}).get("oversampling_topology_k"),
    )

    res = _audit_keel_file_base(
        file_path,
        out_dir=out_dir,
        save=save,
        tolerance=tolerance,
        near_constant_threshold=float(settings["near_constant_threshold"]),
        high_missingness_threshold=float(settings["high_missingness_threshold"]),
        high_cardinality_threshold=int(settings["high_cardinality_threshold"]),
        robust_z_threshold=float(settings["robust_z_threshold"]),
        oversampling_analysis_max_rows=int(settings["oversampling_analysis_max_rows"]),
        oversampling_feasibility_ks=list(settings["oversampling_feasibility_ks"]),
        oversampling_topology_k=int(settings["oversampling_topology_k"]),
    )
    if res.get("status") == "error":
        return res

    _, attributes, inputs, outputs, df = _load_keel_dataframe_for_enhanced_checks(file_path)
    ccm = _compute_class_conditional_missingness(
        df,
        inputs=inputs,
        outputs=outputs,
        gap_threshold=float(settings["class_conditional_missingness_gap_threshold"]),
    )
    redundancy = _compute_feature_redundancy_checks(
        df,
        attributes=attributes,
        inputs=inputs,
        corr_threshold=float(settings["feature_redundancy_corr_threshold"]),
        duplicate_ratio_threshold=float(settings["feature_redundancy_duplicate_ratio_threshold"]),
    )
    target_leakage = _compute_target_leakage_suspicion(
        df,
        attributes=attributes,
        inputs=inputs,
        outputs=outputs,
        suspicion_threshold=float(settings["target_leakage_suspicion_threshold"]),
    )
    metric_ready = _compute_metric_readiness(
        class_info=res.get("class_info", {}),
        oversampling_audit=res.get("oversampling_audit", {}),
        partition_stability=None,
        min_minority_count_pr=int(settings["metric_min_minority_count_pr"]),
        min_minority_count_f1=int(settings["metric_min_minority_count_f1"]),
    )

    res["audit_version"] = "1.4"
    res["audit_configuration"] = {
        "audit_profile": settings.get("audit_profile", "balanced"),
        "profile_overrides": profile_overrides or (ctx or {}).get("profile_overrides") or {},
        "resolved_thresholds": {k: v for k, v in settings.items() if k != "audit_profile"},
    }
    res.setdefault("data_quality", {})
    res["data_quality"]["class_conditional_missingness"] = ccm
    res.setdefault("feature_quality", {})
    res["feature_quality"]["redundancy"] = redundancy
    res["target_leakage_suspicion"] = target_leakage
    res["metric_readiness"] = metric_ready

    if save and isinstance(res.get("saved_to"), str):
        _save_json(res["saved_to"], res)
    return res


def _postprocess_folder_result(result: Dict[str, Any]) -> Dict[str, Any]:
    """
    @brief Add new high-level lists and counters after folder audit.
    """
    rows = result.get("executive_dataset_summaries", [])
    hl_flags = result.setdefault("high_level_flags", {})
    hl_lists = result.setdefault("high_level_lists", {})

    mappings = {
        "datasets_with_near_duplicate_leakage": "has_near_duplicate_leakage",
        "datasets_with_feature_distribution_shift": "has_feature_distribution_shift",
        "datasets_with_class_conditional_missingness_gap": "has_class_conditional_missingness_gap",
        "datasets_with_feature_redundancy": "has_feature_redundancy",
        "datasets_with_minority_fragmentation": "has_minority_fragmentation",
        "datasets_with_unseen_test_categories": "has_unseen_test_categories",
        "datasets_with_target_leakage_suspicion": "has_target_leakage_suspicion",
        "datasets_with_high_majority_noise_suspicion": "has_high_majority_noise_suspicion",
        "datasets_with_metric_readiness_risk": "has_metric_readiness_risk",
    }

    for out_name, flag_name in mappings.items():
        vals = sorted(list({r.get("dataset_key") for r in rows if r.get("flags", {}).get(flag_name)}))
        hl_lists[out_name] = vals
        hl_flags[f"{out_name}_any"] = bool(vals)

    result["audit_version"] = "1.4"
    return result
...
def audit_keel_folder(
    ...
    *,
    ...
    """
    @brief Enhanced folder audit with profile-aware nested file audits.
    """
    settings = _resolve_audit_settings(audit_profile=audit_profile, profile_overrides=profile_overrides)
    settings["profile_overrides"] = profile_overrides or {}
    _push_audit_context(settings)
    try:
        res = _audit_keel_folder_base(folder_path, out_dir=out_dir, save=save)
    finally:
        _pop_audit_context()
    res = _postprocess_folder_result(res)
    if save and isinstance(res.get("saved_to"), str):
        _save_json(res["saved_to"], res)
    return res
...
def audit_keel_root(
    ...
    *,
    ...
    """
    @brief Enhanced root audit with propagation of profile-aware settings.
    """
    settings = _resolve_audit_settings(audit_profile=audit_profile, profile_overrides=profile_overrides)
    settings["profile_overrides"] = profile_overrides or {}
    _push_audit_context(settings)
    try:
        res = _audit_keel_root_base(root_folder, out_dir=out_dir, save=save, top_n_offenders=top_n_offenders)
    finally:
        _pop_audit_context()

    per_dataset = res.get("executive_summary", {}).get("per_dataset", [])
    global_summary = res.setdefault("global_summary", {})
    issue_counters = res.setdefault("executive_summary", {}).setdefault("issue_counters", {})
    mappings = {
        "datasets_with_near_duplicate_leakage": "has_near_duplicate_leakage",
        "datasets_with_feature_distribution_shift": "has_feature_distribution_shift",
        "datasets_with_class_conditional_missingness_gap": "has_class_conditional_missingness_gap",
    ...
    for out_name, flag_name in mappings.items():
        vals = [
            {"folder": r.get("folder"), "dataset_key": r.get("dataset_key")}
            for r in per_dataset if r.get("flags", {}).get(flag_name)
        ]
        global_summary[out_name] = vals
        issue_counters[flag_name] = int(len(vals))
